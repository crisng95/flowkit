...
| Shot | `GET/POST /scenes/{sid}/shots`, `PATCH/DELETE /shots/{sid}`, `…/shots/{sid}/image`, `…/video`, `…/video/resume`, `…/prompts`, `…/upscale`, `…/insert`, `POST /scenes/{sid}/shots/bulk` (nhiều dòng → nhiều shot, `field` = `description` ảnh / `motion_prompt` video), `POST /projects/{pid}/shots/generate-all` (theo lô 3 clip, `GET …/generate-all/preview` đếm trước) |

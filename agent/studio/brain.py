...
_CINE_CONTINUOUS = (
    ...
    "  • Nobody materialises. A person who appears in ANY frame of this scene and stays put — a "
    "vendor at their pitch, someone seated, a shopkeeper in a doorway — has been standing in "
    "that same spot for the WHOLE scene, so every frame whose view covers that spot must show "
    "them. Two frames sharing the same angle cannot differ by a person appearing out of "
    "nowhere. If someone genuinely arrives partway through, write their entrance into the frame "
    "where it happens.\n"

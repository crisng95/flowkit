...
Why not L5/L6/L7 yet:

- several core reference repositories have been audited;
- high-level subsystem boundaries are understood;
- a candidate architecture exists;
- however security, deployment, cost/concurrency targets, persistence technology, formal API contracts, migration model, full failure matrix, and requirement→test traceability are not yet complete;
- therefore the architecture is not yet frozen.

...
Evidence still required before L6:

```text
1. Execute Windows persistence/update harness on target Windows.
2. Execute Windows Electron credential-broker spike.
3. Execute live provider ambiguity proof for first shipping provider.
4. Run real-image Static QA calibration.
5. Run real-generation Targeted Repair benchmark.
6. Collect representative provider quota/rate-limit/cost evidence.
7. Independent review 67..N.
```

Architecture remains L5.5 until these external/target-runtime gates are closed.

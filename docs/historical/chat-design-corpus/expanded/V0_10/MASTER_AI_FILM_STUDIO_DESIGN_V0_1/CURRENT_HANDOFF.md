...
**Last completed action:** Completed provider capability/quota/billing research, cross-document contradiction audit, orthogonal job-state patch, typed billing model, and reviews 75–84.
...
Continue by evidence availability, without production feature coding:

```text
A. Execute target Windows persistence/update harness when Windows execution is available.
B. Execute Electron credential-broker live spike when target Electron runtime is available.
C. Execute live provider ambiguity proof when authorized provider test access is available.
D. Execute real-image Static QA calibration when the labeled image set is available.
E. Execute real-generation repair benchmark when provider access + failed-shot set are available.
F. Continue exact shipping-provider profile verification and final contradiction/traceability review.
```

Do not promote to L6/L7 without the external evidence.

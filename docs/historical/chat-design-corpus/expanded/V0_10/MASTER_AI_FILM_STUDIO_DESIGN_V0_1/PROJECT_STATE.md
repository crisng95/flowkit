...
- Cost model uses typed billing units, not universal price/job.
...
Remaining L6 evidence gates are now sharply bounded:

```text
1. Target Windows persistence/update harness execution.
2. Target Windows Electron safeStorage/utilityProcess proof.
3. Live ambiguity fault injection on first shipping provider.
4. 240-image real Static QA calibration.
5. 90-failure real-generation Targeted Repair benchmark.
6. Live/account quota/cost observations for shipping provider profiles.
7. Independent review 85..N after evidence.
```

Current design remains L5.5; conceptual provider/document contradictions are patched.

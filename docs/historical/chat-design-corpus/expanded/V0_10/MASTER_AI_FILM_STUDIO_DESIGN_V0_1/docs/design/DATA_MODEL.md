...
V0.10 uses orthogonal state axes.
...
The deprecated generic `status` / overloaded `provider_job_id` semantics must not be used as the only source of truth.
...
Do not compare Flow credits to USD without an explicit conversion policy.

...
**Decision:** private Flow transport, if retained experimentally, gets a separate profile and evidence grade.
...
### HIGH — R76-F02
Documented model limits can still differ from account-specific live limits.

**Decision:** allow runtime account-observed override/backpressure while preserving evidence provenance.
...
### CRITICAL — R77-F01
A universal `price_per_job` cannot represent Flow credits, per-second Wan pricing, token-priced Seedance, provisioned throughput or local GPU compute.

**Decision:** typed billing unit model accepted.

### HIGH — R77-F02
Credits and USD must not be numerically compared without an explicit conversion policy.

**Decision:** store unit/currency separately; budget rules operate within explicit unit domains unless conversion is configured.
...
### HIGH — R78-F01
Provider task handles have different retention windows.

**Evidence examples:** current BytePlus task surface documents 7-day task-ID retention; current Alibaba Wan legacy task docs state task ID valid 24 hours.

**Decision:** `reconciliation_deadline` is provider-profile data.

### HIGH — R78-F02
Recovery after the provider handle expires cannot be treated as normal polling.

**Decision:** transition to `EXPIRED_UNKNOWN`/manual evidence path according to adapter policy.
...
### CRITICAL — R79-F01
A generic `cancel()` API can falsely imply that running remote work stopped.

**Decision:** expose cancel-queued, cancel-running and delete-terminal capability separately.

### HIGH — R79-F02
Local `CANCEL_REQUESTED` is intent, not proof of remote cancellation.
...
### HIGH — R80-F01
Google Flow product credits are charged per generation rather than necessarily per user request.

**Decision:** Flow cost estimator tracks requested candidate count and actual generation count.

### HIGH — R80-F02
Flow product rate limits are adaptive; a fixed RPM constant is not supported by current product help.

**Decision:** Flow Product admission uses conservative configurable caps plus live backpressure, not a fictitious official RPM.

### CRITICAL — R80-F03
Private/internal Flow transport is not equivalent to a supported public developer API.

**Decision:** any private Flow adapter remains experimental and cannot be used as the architectural contract for direct Veo APIs.
...
### CRITICAL — R81-F01
Applying Alibaba Cloud Model Studio quotas/pricing to local Wan execution would be a category error.

**Decision:** split `WanLocalAdapter` and `AlibabaModelStudioWanAdapter`.

### HIGH — R81-F02
Local Wan capacity is hardware/resource-pool driven.

**Decision:** local adapter admission is GPU/VRAM/model-residency/CPU/disk based.
...
### CRITICAL — R82-F01
A single status enum conflates scheduler ownership, provider acceptance, artifact materialization and creative approval.

**Decision:** four orthogonal state axes adopted.

### HIGH — R82-F02
UI still needs a simple status.
...
### HIGH — R83-F01
Baseline V0.1 documents retained stale L4 wording after the design advanced.

**Decision:** current maturity headers patched; `PROJECT_STATE.md` remains single maturity authority.

### HIGH — R83-F02
Historical review statements must not be rewritten as if they were never true.

**Decision:** keep review history intact; patch current authority documents instead of erasing historical findings.
...
### CRITICAL — R84-F01
Provider research adds stronger evidence but still does not close live-provider, Windows, visual-QA or real-repair gates.

**Decision:** remain L5.5.

### HIGH — R84-F02
The design now has more provider-specific detail; this must not leak provider syntax upward into ShotIR.

**Decision:** ProviderProfile affects routing/compilation/runtime only; canonical Story/ShotSpec/ShotIR remain provider-neutral.

### POSITIVE
Cross-document contradictions identified in V0.10 have explicit patches and a canonical decision registry.

---

# Review status after Round 84

**Unresolved conceptual contradiction BLOCKER:** 0 after patches.

**External/runtime evidence blockers for L6 remain:**
- target Windows persistence/update execution;
- target Windows Electron credential-broker execution;
- live provider ambiguity proof;
- real-image Static QA calibration;
- real-generation Targeted Repair benchmark;
- provider/account live quota/cost observations where documentation is dynamic.

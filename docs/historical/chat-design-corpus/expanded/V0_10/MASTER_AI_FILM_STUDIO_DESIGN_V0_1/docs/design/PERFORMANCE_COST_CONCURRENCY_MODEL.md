...
# V0.10 Provider-Evidence Patch

Admission numbers are never hard-coded architecture constants.

Runtime resolution:

```text
ProviderProfile
+ account-specific observed quota
+ live Retry-After/backpressure
...
Current research shows materially different provider/model scopes:
- Google Flow uses credits plus adaptive product rate limiting;
- Vertex Veo model pages publish per-project RPM values for specific model versions;
- BytePlus Seedance publishes model-version RPM/concurrency and can queue above concurrency;
- Alibaba Model Studio Wan publishes model/region RPS and concurrency;
- local Wan is hardware-bound, not remote-quota-bound.

Cost admission also uses typed billing units; there is no universal `price_per_job`.

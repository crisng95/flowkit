...
| Req | Component | Data | Contract | Test | Acceptance |
|---|---|---|---|---|---|
| PERF-007 provider-safe concurrency | Scheduler | ProviderProfile + active counts | admission policy | flat vs hierarchical simulation | no configured cap violation |
| PERF-008 project fairness | Scheduler | queued jobs | scheduling policy | multi-project synthetic load | no persistent starvation under equal priority |
| PERF-009 budget admission | Scheduler/Cost | estimate/actual cost | budget policy | hard-budget simulation | new submissions block at hard budget |

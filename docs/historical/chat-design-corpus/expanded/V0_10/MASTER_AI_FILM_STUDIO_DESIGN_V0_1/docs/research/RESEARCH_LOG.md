...
## R-017 — Credential Broker
**Sources:** current official Electron `safeStorage`, `utilityProcess`, process-model, context-isolation and security documentation + local protocol/security simulation.  
**Finding:** Main is the correct privileged boundary for OS-backed safeStorage; Renderer must never receive a raw secret; Utility can receive a short-lived scoped secret only if it remains a trusted first-party process.  
**Evidence:** local simulation passed renderer denial, utility authentication, provider/operation scoping, replay/expiry, rotation invalidation, utility-crash revocation, vault/export/log canary scans.  
**Limitation:** sandbox has no Electron executable and is not Windows; DPAPI/safeStorage live behavior remains unproven.  
**Decision:** scoped one-use credential lease architecture accepted as reviewed candidate.


## R-018 — Static QA policy spike
**Evidence:** `STATIC_QA_POLICY_SPIKE_EVIDENCE_V0_7.md`.  
**Result:** weighted-average acceptance passed 60/60 synthetic blocking-defect cases; severity-based policy passed 0 of those failures.  
**Decision:** blocking defects cannot be averaged away.  
**Limitation:** no real image/vision-model detection evidence.

## R-019 — Targeted Repair planner spike
**Evidence:** `TARGETED_REPAIR_PLANNER_SPIKE_EVIDENCE_V0_7.md`.  
**Result:** 180 planner cases, zero preserve/owner invariant violations; synthetic logical invalidation scope averaged ~48% below full-regenerate reference model.  
**Decision:** typed RepairPlan + selective invalidation retained.  
**Limitation:** no proof of actual generative repair success or monetary savings.


## R-020 — Performance / concurrency local spike
**Evidence:** `PERFORMANCE_COST_CONCURRENCY_SPIKE_EVIDENCE_V0_8.md`.  
**Result:** 300-shot/314-edge/300-job metadata workload remained cheap in the current sandbox; a flat global-only scheduler violated synthetic provider caps while hierarchical global/provider/model admission produced zero configured cap violations.  
**Decision:** retain modular-monolith/SQLite candidate and hierarchical admission architecture.  
**Limitation:** provider capacities/durations/costs were synthetic; target Windows and live provider evidence remain open.

## R-021 — Windows persistence/update harness
**Artifact:** `tools/design-spikes/windows-persistence-update/`.  
**Purpose:** make target Windows proof reproducible for WAL concurrency, abrupt process exit, migration interruption/resume, backup/restore and newer-schema write gate.  
**Status:** harness created, not executed on target Windows.


## R-022 — Provider capability/quota/billing evidence matrix
**Evidence:** official Google Flow help, Google Veo Gemini/Vertex docs, BytePlus ModelArk Seedance docs/pricing/task API, Alibaba Model Studio Wan API/rate-limit/pricing docs, official Wan repository.  
**Finding:** provider brand alone is insufficient identity; quotas, billing, task retention and recovery semantics vary by product surface/model/version/region/account.  
**Decision:** ProviderProfile now includes provider surface, region, billing unit, task lifecycle, recovery and evidence metadata.

## R-023 — Cross-document contradiction audit
**Evidence:** `CROSS_DOCUMENT_CONTRADICTION_AUDIT_V0_10.md` + static consistency scanner.  
**Finding:** stale maturity headers, old monolithic job state, provider-surface conflation and overly generic cost/cancel semantics were identified.  
**Decision:** patch current authority docs; adopt orthogonal job states, typed billing units and canonical decision registry.

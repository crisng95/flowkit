...
**Evidence:** current official Google Flow/Veo, BytePlus Seedance, Alibaba Model Studio Wan and official Wan repository sources.
**Finding:** provider brand alone is insufficient identity; runtime facts vary by surface/model/version/region/account.
**Decision:** ProviderProfile is evidence-versioned and surface-specific.

## R-023 — Cross-document contradiction audit
**Finding:** stale maturity headers, monolithic job state, provider-surface conflation, generic cost/cancel semantics and traceability-ID drift were identified.
**Decision:** patch current authority docs, add orthogonal states, typed billing, decision registry and traceability validation.

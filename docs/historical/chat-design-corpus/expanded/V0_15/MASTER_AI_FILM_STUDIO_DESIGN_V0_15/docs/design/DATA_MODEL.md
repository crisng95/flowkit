...
Artifact materialization statuses:
...
Creative `APPROVED` / `REJECTED` are stored on the creative acceptance axis, not on artifact materialization status.
...
## V0.13 Job-State Clarification

`GenerationJob` state enums follow `JOB_STATE_MODEL_V0_13.md`, including scheduler `WAITING_RECOVERY` and creative `QA_ERROR`.

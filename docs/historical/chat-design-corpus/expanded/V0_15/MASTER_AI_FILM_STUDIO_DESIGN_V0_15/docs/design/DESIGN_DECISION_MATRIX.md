...
## V0.3 Security / Contracts / Deployment Decisions

| Decision | A | B | C | Candidate |
|---|---|---|---|---|
| Desktop backend topology | Electron main only | Main broker + utilityProcess backend | localhost service | **B** |
| Renderer privilege | broad IPC | narrow capability IPC | Node integration | **narrow IPC** |
| Secret storage | project/env plaintext | main-process OS-backed safeStorage vault | renderer store | **safeStorage vault** |
| Runtime contracts | TS interfaces | JSON Schema only | Zod 4 + JSON Schema export | **Zod + JSON Schema** |
| Observability | console only | durable events + structured logs + optional OTel | mandatory collector | **events/logs + optional OTel** |
| External contract dialect | custom | JSON Schema 2020-12 | OpenAPI-only | **JSON Schema 2020-12** |

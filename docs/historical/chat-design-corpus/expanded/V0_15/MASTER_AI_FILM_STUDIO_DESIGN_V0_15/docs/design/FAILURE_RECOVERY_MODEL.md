...
Adapters expose `cancel_queued`, `cancel_running`, and `delete_terminal` separately. `CANCEL_REQUESTED` is local intent only; only provider-confirmed evidence yields `REMOTE_CANCELED`.

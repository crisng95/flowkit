...
Admission numbers are not architecture constants. Runtime resolution is:

```text
ProviderProfile + account-observed quota + live backpressure + local override policy → AdmissionPolicy
```

Earlier numeric examples are illustrative only. Cost admission uses typed billing units; there is no universal price-per-job.

...
Only artifacts whose **creative acceptance state** is `APPROVED` or `LOCKED`, and whose artifact materialization state is usable (`READY`), can become default parent conditioning.
...
# V0.7 Policy Spike Result

Synthetic adversarial policy fixtures proved a key architectural rule:

```text
weighted aggregate alone can mask a BLOCKING failure
```

In the test set, 60 blocking-defect cases were deliberately given high scores on every unrelated dimension. The weighted-average policy passed all 60 at the tested threshold, while the severity gate failed all 60.

This is **policy evidence only**.

It does not prove the vision reviewer will detect those defects in real images.

Therefore Static QA has two independent maturity tracks:

```text
A. ACCEPTANCE POLICY
   severity gate / no blocking average masking

B. SENSOR QUALITY
   real labeled image calibration
   precision/recall per defect
```

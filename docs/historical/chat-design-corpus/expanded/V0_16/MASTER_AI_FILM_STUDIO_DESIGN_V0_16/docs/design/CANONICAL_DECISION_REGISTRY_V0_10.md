...
| D-009 | Persistence | SQLite WAL + FULL with one logical write owner / bounded write queue in Production Utility | TARGET-RUNTIME PROVEN DESIGN DIRECTION |
...
| D-025 | Freeze | Implementation baseline may freeze after design/traceability/task readiness; live subsystem evidence belongs to implementation/release acceptance unless it is an architecture-changing pre-code spike | PROCESS CORRECTED / ADR-0015 |
...
| D-028 | Story Intelligence | Studio-owned canonical Premise→Angle→Theme→Research→Brain→Character→Conflict→Causality→Emotion→Scene/Beat→Dialogue→Critique/Repair→Script Lock system | REVIEWED CANDIDATE / 12-ROUND AUDITED | ADR-0014 |
| D-029 | Story repo reuse | Third-party repos are donors through license+adapter/port/Brain-Pack boundaries; no donor owns canonical story truth | REVIEWED CANDIDATE | ADR-0014 |
| D-030 | Story memory authority | SQLite canonical Story Domain is authority; semantic/vector/memory frameworks are derived retrieval support only | REVIEWED CANDIDATE | ADR-0014 |
| D-031 | Story quality | Blocking story defects cannot be averaged away; specialist critique + targeted repair | REVIEWED CANDIDATE | STORY_QUALITY_CRITIQUE_REPAIR_V0_16 |

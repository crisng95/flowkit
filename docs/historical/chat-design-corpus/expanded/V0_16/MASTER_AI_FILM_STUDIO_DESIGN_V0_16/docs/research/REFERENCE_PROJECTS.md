...
# V0.16 Story Intelligence donor shortlist

See `STORY_REPO_SOURCE_SNAPSHOT_V0_16.json` for pinned commits and `STORY_REPO_DECISION_MATRIX_V0_16.md` for legal/integration decisions.

Primary donors/patterns audited:
- Dramatron — hierarchical screenplay generation/intervention history.
- Sabre — intentional narrative planning, belief/utility/causal state (pattern-only by default due GPL).
- StoryDaemon — beat-first planning, rolling horizon, tension and selective repair (pattern-only; no license detected).
- Beatlume — scene scaffolding/relationship and deterministic tension/pacing analytics (pattern-only; no license detected).
- GPT Researcher — recursive research, reviewer/reviser orchestration.
- STORM — perspective/persona-guided knowledge curation and outline refinement.
- Evidence Graph — atomic claims, contradiction/skeptic loops.
- Open-Write — screenplay/story production pipeline, blinded critics, state/callback/audience ledgers, completion verification.
- SOTOPIA — character/social-goal profiles and evaluation patterns.
- Mem0 — optional semantic memory adapter only, never canonical story authority.
- Craft Brain Packs — Writers Room Story Engine, Story Architect Skill, Script Doctor.
